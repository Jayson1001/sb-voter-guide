...
export default function ElectionHomePage({ params }: { params: { electionSlug: string } }) {
  ...
  const { election, results, ballotArticles = [], races } = data;
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <div className="flex items-center gap-2 text-sm text-white/70 mb-6 flex-wrap">
            <Link href="/" className="hover:text-white transition-colors font-medium">Election Guide</Link>
            <svg className="w-3 h-3 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            <span className="text-white/90">{election.name}</span>
          </div>

          <div className="text-center animate-fade-in">
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-4 tracking-tight">
              {election.location.split(',')[0]}
              <span className="block text-cream-200">Election Results {new Date(election.date + 'T12:00:00').getFullYear()}</span>
            </h1>

            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 mt-6 text-cream-100">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full">
                <CalendarIcon />
                <span className="font-medium">{electionDate}</span>
              </div>
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full">
                <MapPinIcon />
                <span className="font-medium">{election.location}</span>
              </div>
            </div>
              ...
              <div className="mt-6">
                <div className="inline-flex items-center gap-3 bg-white/15 backdrop-blur-sm px-5 py-3 rounded-xl">
                  <div className="text-cream-100 font-display font-medium text-base">
                    Town Meeting Day results are in &mdash; {results.totalBallotsCast.toLocaleString()} ballots cast ({results.turnoutPercent}% turnout)
                  </div>
                </div>
              </div>
            )}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-6 animate-fade-in-delay-1">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="font-display text-xl font-bold text-warmgray-800 mb-2">The Results Are In</h2>
                        ...
                        <div className="flex items-center gap-2">
                          {candidate.elected ? (
                            <span className="text-xs font-semibold text-sage-700 bg-sage-100 px-2 py-0.5 rounded-full">
                              Elected &mdash; {candidate.votes?.toLocaleString()} votes
                            </span>
                          ) : (
                            <span className="text-xs font-semibold text-warmgray-500 bg-warmgray-100 px-2 py-0.5 rounded-full">
        ...
        {/* Ballot Articles */}
        {ballotArticles.length > 1 && (
          <div className="mb-12">
            <div className="text-center mb-8">
              <h2 className="font-display text-2xl sm:text-3xl font-bold text-warmgray-800 mb-2">Ballot Article Results</h2>
              <p className="text-warmgray-500">How {election.location.split(',')[0]} voted on {electionDate}</p>
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-display font-bold text-xl">{electionDate} &mdash; Final Results</p>
                  </div>
                </div>
                <a
        ...
        <footer className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 text-center">
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-warmgray-600 text-sm leading-relaxed mb-3">
              This voter guide was created to help {election.location.split(',')[0]} residents make informed decisions.
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4 border-t border-warmgray-100">
              <Link
                href={`/elections/${electionSlug}/sources`}
