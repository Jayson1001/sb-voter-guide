...
import { getCandidateById, getAllCandidateSlugs } from "@/lib/elections";
...
export function generateStaticParams() {
  return getAllCandidateSlugs();
}

export default function CandidatePage({ params }: { params: { electionSlug: string; id: string } }) {
  const { electionSlug, id } = params;

  const result = getCandidateById(electionSlug, id);
  if (!result) notFound();

  const { candidate, raceTitle, candidateIndex } = result;
    ...
    <main className="min-h-screen bg-pattern">
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          <Nav electionSlug={electionSlug} electionName={electionData.election.name} currentPage={candidate.name} />
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="font-display text-3xl sm:text-4xl font-bold text-white mb-1">{candidate.name}</h1>
              <p className="text-white/80">Candidate for {raceTitle}</p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <section className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-6 animate-fade-in">
          ...
          {candidate.experience && candidate.experience.length > 0 && (
            <div className={`border-t pt-6 ${accentColor === 'sage' ? 'border-sage-100' : 'border-terracotta-100'}`}>
              <h3 className="font-display font-semibold text-warmgray-800 mb-4">Experience & Involvement</h3>
        ...
        {Object.keys(candidate.positions).length > 0 && (
          <section className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-6 animate-fade-in-delay-1">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="font-display text-xl font-bold text-warmgray-800">Positions on Key Issues</h2>
            </div>
              ...
              {Object.entries(candidate.positions).map(([key, position]) => (
                <div key={key} className={`rounded-xl p-5 ${
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <span className={accentColor === 'sage' ? 'text-sage-600' : 'text-terracotta-600'}>
                      {issueIcons[key] || issueIcons.governance}
                    </span>
                    <h3 className="font-display font-semibold text-warmgray-800">{position.title}</h3>
                  </div>
                  <p className="text-warmgray-600 leading-relaxed pl-8">{position.stance}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        {((candidate.civicRecord && candidate.civicRecord.length > 0) || candidate.civicRecordNote) && (
          <section className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-6 animate-fade-in-delay-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="font-display text-xl font-bold text-warmgray-800">Civic Record</h2>
        ...
        {candidate.website && (
          <section className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-6 animate-fade-in-delay-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="font-display text-xl font-bold text-warmgray-800">Campaign Website</h2>
            </div>
            <a
              ...
              className={`inline-flex items-center gap-2 font-medium transition-colors ${
                accentColor === 'sage' ? 'text-sage-600 hover:text-sage-700' : 'text-terracotta-600 hover:text-terracotta-700'
              }`}
            >
              {candidate.website} <LinkIcon />
            </a>
          </section>
        )}
        ...
        {candidate.sources && candidate.sources.length > 0 && (
          <section className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 mb-8 animate-fade-in-delay-3">
            <h2 className="font-display text-lg font-bold text-warmgray-800 mb-4">Sources & References</h2>
        ...
        <div className="text-center">
          <Link
            ...
            className={`inline-flex items-center gap-2 px-8 py-4 rounded-xl font-display font-semibold text-white transition-all hover:scale-105 hover:shadow-lg ${
              accentColor === 'sage' ? 'bg-sage-500 hover:bg-sage-600' : 'bg-terracotta-500 hover:bg-terracotta-600'
            }`}
          >
            <ArrowLeftIcon />
